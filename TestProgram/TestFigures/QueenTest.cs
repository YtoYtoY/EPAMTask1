...
namespace TestProgram.TestFigures
{
    ...
    public class QueenTest
    {
        ...
        public void ForTest()
        {
            ...
            board.AddFigure(queen, new Position(1, 'c'));
            players[1].AddFigure(queen);
        ...
        [TestMethod]
        public void QueenFalseMovable_UnitTest()
        {
            Information.AddLog("***** Testing wrong move queen *****");
            ...
            catch (Exception ex)
            {
                ...
                Assert.AreEqual(board.GetFigureAtPosition(new Position(1, 'c')), queen);
                Assert.AreEqual(board.GetFigureAtPosition(new Position(8, 'f')), pawn);
            }
        }

        [TestMethod]
        public void QueenTrueMovableAndAttack_UnitTest()
        {
            Information.AddLog("***** Testing correct move queen *****");
            var game = new GameFunction();
            ForTest();
            IFigure figure;
            string[] moveCommands = { "c1-f4", "f4-f8" };
            int index = 0;
            while (index < 2)
            {
            ...
            Assert.AreEqual(board.GetFigureAtPosition(new Position(8, 'f')), queen);
        }
    }
}
